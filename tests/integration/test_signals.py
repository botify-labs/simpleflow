...
class TestSignals(VCRIntegrationTest):
    ...
    def test_unrequested_signal(self):
        ...
        expect(
            events[-1]["workflowExecutionCompletedEventAttributes"]["result"]
        ).should.be.equal('"signal sent!"')
        n = 0
        for e in events:
    ...
    def test_wait_signal(self):
        ...
        events = self.run_standalone(
            "tests.integration.workflow.WorkflowWithWaitSignal"
        )
        expect(events[-1]["eventType"]).should.be.equal(
        ...
        expect(events[4]["decisionTaskCompletedEventAttributes"]).should.contain(
            "executionContext"
        )
        execution_context = json.loads(
            events[4]["decisionTaskCompletedEventAttributes"]["executionContext"]
        )
        ...
        expect(events[14]["decisionTaskCompletedEventAttributes"]).should.contain(
            "executionContext"
        )
        execution_context = json.loads(
            events[14]["decisionTaskCompletedEventAttributes"]["executionContext"]
        )
        ...
        expect(events[22]["decisionTaskCompletedEventAttributes"]).should.contain(
            "executionContext"
        )
        execution_context = events[22]["decisionTaskCompletedEventAttributes"][
            "executionContext"
        ]
        ...
        expect(events[24]["decisionTaskCompletedEventAttributes"]).should_not.contain(
            "executionContext"
        )
