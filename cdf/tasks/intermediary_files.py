...
def make_links_counter_file(crawl_id, s3_uri,
    ...
    stream_links = stream_name().get_stream_from_s3(s3_uri, tmp_dir, part_id, force_fetch)
...
def make_bad_link_file(crawl_id, s3_uri,
    ...
    for part_id in xrange(0, nb_parts):
        ...
        streams_types['infos'].append(InfosStreamDef.get_stream_from_s3(**storage_kwargs))
        streams_types['outlinks'].append(OutlinksStreamDef.get_stream_from_s3(**storage_kwargs))
...
def make_bad_link_counter_file(crawl_id, s3_uri,
    ...
    """
    stream = BadLinksStreamDef.get_stream_from_s3(s3_uri, tmp_dir=tmp_dir, force_fetch=force_fetch, part_id=part_id)
