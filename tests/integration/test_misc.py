...
class TestMisc(VCRIntegrationTest):
    ...
    def test_cancel_requested(self):
        events = self.run_standalone(
            "tests.integration.workflow.WorkflowToCancel", True
        )
        cancel_requested = [
            e for e in events if e["eventType"] == "WorkflowExecutionCancelRequested"
        ]
    ...
    def test_cancel_requested_refused(self):
        events = self.run_standalone(
            "tests.integration.workflow.WorkflowToCancel", False
        )
        cancel_requested = [
            e for e in events if e["eventType"] == "WorkflowExecutionCancelRequested"
        ]
