...
class _Result(collections.Iterable):
    """
    Redirections' final destinations, internal format.
    """
    def __init__(self):
        try:
    ...
    def __iter__(self):
        for uid, dst in self.uid_to_dst.iteritems():
            ext = self.uid_to_ext.get(uid, None)
            in_loop = uid in self.uid_in_loop
            if ext is None and not in_loop:
...
class FinalRedirectionStreamDef(StreamDefBase):
    ...
    @staticmethod
    def compute_final_redirections(stream_infos, stream_links):
        """Compute, for each "rxxx" link, its final URL along with the hop count and whether it's in a loop.
