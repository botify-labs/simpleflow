...
class TestMarkers(VCRIntegrationTest):
    ...
    def test_without_replays(self):
        ...
        expect(marker_details).to.equal(
            [None, '"some details"', '"2nd marker\'s details"']
        )
        decision_task_completed_event_id = {
            e["markerRecordedEventAttributes"]["decisionTaskCompletedEventId"]
            for e in marker_recorded
        }
    ...
    def test_with_replays(self):
        ...
        expect(len(decision_task_completed_event_id)).to.equal(
            3
        )  # sent in different decisions
