...
def make_links_to_not_strategic_file(s3_uri,
    ...
    generator = get_links_to_not_strategic_urls(
        StrategicUrlStreamDef.load(**stream_kwargs),
        OutlinksStreamDef.load(**stream_kwargs)
    )

    result = LinksToNotStrategicStreamDef.persist(
        generator, s3_uri,
        first_part_size=first_part_id_size,
        part_size=part_id_size
    )
...
def make_links_to_not_strategic_counter_file(s3_uri,
                                             ...
                                             force_fetch=DEFAULT_FORCE_FETCH):
    """
    Generate a counter file that list link to not strategic url counts by source url
      url_src_id  count

    This method depend on the file generated by `make_links_to_not_strategic_file`
    Ordered on url_src_id and http_code
    :param s3_uri: the s3 uri where the crawl data is stored.
    :type s3_uri: str
    ...
    :returns: list - the list of generated files
    """
    stream = LinksToNotStrategicStreamDef.load(
        s3_uri,
        tmp_dir=tmp_dir,
        part_id=part_id,
        force_fetch=force_fetch
