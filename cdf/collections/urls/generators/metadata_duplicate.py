...
class MetadataDuplicateGenerator(object):

    def __init__(self, stream_patterns, stream_contents):
    ...
    def __iter__(self):
        ...
        for result in group_left((self.stream_patterns, 0), **streams_def):
            url_id = result[0]
        ...
        for ct_id, ct_hashes in hashes.iteritems():
            for _h in ct_hashes:
                if len(ct_hashes[_h]) > 1:
                    sample = list(ct_hashes[_h])[:11]
                    for url_id in ct_hashes[_h]:
                        yield (CONTENT_TYPE_INDEX[ct_id], url_id, filter(lambda i: i != url_id, sample)[:10])
